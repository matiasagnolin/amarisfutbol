...
def conferenceAppGitUrl="https://github.com/matiasagnolin/amarisfutbol"
createDockerJob("docker-futbol-app-build-container", "cd app && sudo /usr/bin/docker build -t futbolapp .", conferenceAppGitUrl)
createDockerJob("docker-futbol-app-start-container", "sudo /usr/bin/docker run -d --name conferenceapp -p=48080:8080 conferenceapp", conferenceAppGitUrl)
createDockerJob("docker-futbol-app-stop-container", 'sudo /usr/bin/docker stop \$(sudo /usr/bin/docker ps -a -q --filter="name=conferenceapp") && sudo /usr/bin/docker rm \$(sudo /usr/bin/docker ps -a -q --filter="name=conferenceapp")', " ")
...
def createDockerJob(def jobName, def shellCommand, def gitRepository) {
  ...
  job(jobName) {
    ...
    if( "${gitRepository}".size() > 0 ) {
      if( "${jobName}".contains("futbol-app") ) {
        scm {
